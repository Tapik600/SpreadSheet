...
//
// Case analysis:
// A + (B + C) - always okay (nothing of lower grammatic precedence could have been written to
// the right)
//    (e.g. if we had A + (B + C) / D, it wouldn't parse in a way
//    that woudld have given us A + (B + C) as a subexpression to deal with)
// A + (B - C) - always okay (nothing of lower grammatic precedence could have been written to
// the right) A - (B + C) - never okay A - (B - C) - never okay A * (B * C) - always okay (the
// parent has the highest grammatic precedence) A * (B / C) - always okay (the parent has the
// highest grammatic precedence) A / (B * C) - never okay A / (B / C) - never okay
// -(A + B) - never okay
// -(A - B) - never okay
